...
wcl=stim.set_constants(mR[i],k_e,dist_type='weibull')
[_,_,Qscb1]=stim.stim_one(rep_ksn1,cb[i],wcl,sc=sb[i])
[_,_,Qscw1]=stim.stim_one(rep_ksn1,cw[i],wcl,sc=sw[i])
[_,_,Qscb2]=stim.stim_one(rep_ksn2,cb[i],wcl,sc=sb[i])
[_,_,Qscw2]=stim.stim_one(rep_ksn2,cw[i],wcl,sc=sw[i])
[_,_,Qscb3]=stim.stim_one(rep_ksn3,cb[i],wcl,sc=sb[i])
[_,_,Qscw3]=stim.stim_one(rep_ksn3,cw[i],wcl,sc=sw[i])
df=pd.read_csv('data_tables/grdc_discharge_time_series/GRDC_'+str(ID[i])+'.csv')
Q=df['Q'].to_numpy()
[Qs,Qf]=survive(Q)
plt.scatter(Qs*mR[i],Qf,s=20,c='gray',zorder=1)
