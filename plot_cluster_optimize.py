...
perc_carb=(edf['rock_type_perc_limestone_minor_clastics'].to_numpy()+
          edf['rock_type_perc_limestone_marl'].to_numpy()+
          edf['rock_type_perc_limestone'].to_numpy()+
          edf['rock_type_perc_marl'].to_numpy())
perc_clastic=(edf['rock_type_perc_sandstone_shale'].to_numpy()+
              edf['rock_type_perc_sandstone_minor_carbonate'].to_numpy()+
              edf['rock_type_perc_shale'].to_numpy()+
              edf['rock_type_perc_conglomerates'].to_numpy())
perc_mixed=edf['rock_type_perc_limestone_sandstone'].to_numpy()
perc_igmet=(edf['rock_type_perc_phyllite_schist'].to_numpy()+
            edf['rock_type_perc_volcanic_undiff'].to_numpy()+
            edf['rock_type_perc_granite'].to_numpy())
perc_fine=(edf['rock_type_perc_marl'].to_numpy()+
            edf['rock_type_perc_shale'].to_numpy())
perc_ss=edf['rock_type_perc_sandstone_shale'].to_numpy()
...
plt.axhline(2.75,c='k',linestyle=':')
plt.axhline(3.1,c='k',linestyle=':')
plt.axvline(3.5,c='k',linestyle=':')
plt.xlabel('Mean Runoff [mm/day]')
plt.ylabel('Max Elevation [km]')
...
for i in range(num_clustb):
    ax1.errorbar(perc_carb[ecluster_label==i],k_e_optim[ecluster_label==i],
                  yerr=k_e_err[:,ecluster_label==i],ecolor=color_list[i],
                  zorder=0,linestyle='')
    ax1.scatter(perc_carb[ecluster_label==i],k_e_optim[ecluster_label==i],
                c=color_list[i],zorder=1,s=40)
ax1.set_ylabel('Optimized $k_{e}$')
ax1.set_yscale('log')
ax1.set_ylim((10**-13,10**-7))
ax1.set_xlabel('Percent Carbonate')
ax1.set_xlim((-5,105))

ax2=plt.subplot(4,2,2)
for i in range(num_clustb):
    ax2.errorbar(perc_carb[ecluster_label==i],
                  tau_c_optim[ecluster_label==i],yerr=t_c_err[:,ecluster_label==i],
                  ecolor=color_list[i],zorder=0,linestyle='')
    ax2.scatter(perc_carb[ecluster_label==i],tau_c_optim[ecluster_label==i],
                  c=color_list[i],zorder=1,s=40)
ax2.set_ylabel(r'Optimized $\tau_c$')
ax2.set_yscale('log')
ax2.set_ylim((20,90))
ax2.set_xlabel('Percent Carbonate')
ax2.set_xlim((-5,105))

ax3=plt.subplot(4,2,3)
for i in range(num_clustb):
    ax3.errorbar(perc_clastic[ecluster_label==i],k_e_optim[ecluster_label==i],
                  yerr=k_e_err[:,ecluster_label==i],ecolor=color_list[i],
                  zorder=0,linestyle='')
    ax3.scatter(perc_clastic[ecluster_label==i],k_e_optim[ecluster_label==i],
                c=color_list[i],zorder=1,s=40)
ax3.set_ylabel('Optimized $k_{e}$')
ax3.set_yscale('log')
ax3.set_ylim((10**-13,10**-7))
ax3.set_xlabel('Percent Clastic')
ax3.set_xlim((-5,105))

ax4=plt.subplot(4,2,4)
for i in range(num_clustb):
    ax4.errorbar(perc_clastic[ecluster_label==i],
                  tau_c_optim[ecluster_label==i],yerr=t_c_err[:,ecluster_label==i],
                  ecolor=color_list[i],zorder=0,linestyle='')
    ax4.scatter(perc_clastic[ecluster_label==i],tau_c_optim[ecluster_label==i],
                  c=color_list[i],zorder=1,s=40)
ax4.set_ylabel(r'Optimized $\tau_c$')
ax4.set_yscale('log')
ax4.set_ylim((20,90))
ax4.set_xlabel('Percent Clastic')
ax4.set_xlim((-5,105))

ax9=plt.subplot(4,2,5)
for i in range(num_clustb):
    ax9.errorbar(perc_ss[ecluster_label==i],k_e_optim[ecluster_label==i],
                  yerr=k_e_err[:,ecluster_label==i],ecolor=color_list[i],
                  zorder=0,linestyle='')
    ax9.scatter(perc_ss[ecluster_label==i],k_e_optim[ecluster_label==i],
                c=color_list[i],zorder=1,s=40)
ax9.set_ylabel('Optimized $k_{e}$')
ax9.set_yscale('log')
ax9.set_ylim((10**-13,10**-7))
ax9.set_xlabel('Percent Sandstone & Shale')
ax9.set_xlim((-5,105))

ax10=plt.subplot(4,2,6)
for i in range(num_clustb):
    ax10.errorbar(perc_ss[ecluster_label==i],
                  tau_c_optim[ecluster_label==i],yerr=t_c_err[:,ecluster_label==i],
                  ecolor=color_list[i],zorder=0,linestyle='')
    ax10.scatter(perc_ss[ecluster_label==i],tau_c_optim[ecluster_label==i],
                  c=color_list[i],zorder=1,s=40)
ax10.set_ylabel(r'Optimized $\tau_c$')
ax10.set_yscale('log')
ax10.set_ylim((20,90))
ax10.set_xlabel('Percent Sandstone & Shale')
ax10.set_xlim((-5,105))

ax11=plt.subplot(4,2,7)
for i in range(num_clustb):
    ax11.errorbar(perc_igmet[ecluster_label==i],k_e_optim[ecluster_label==i],
                  yerr=k_e_err[:,ecluster_label==i],ecolor=color_list[i],
                  zorder=0,linestyle='')
    ax11.scatter(perc_igmet[ecluster_label==i],k_e_optim[ecluster_label==i],
                c=color_list[i],zorder=1,s=40)
ax11.set_ylabel('Optimized $k_{e}$')
ax11.set_yscale('log')
ax11.set_ylim((10**-13,10**-7))
ax11.set_xlabel('Percent Igneous-Metamorphic')
ax11.set_xlim((-5,105))

ax12=plt.subplot(4,2,8)
for i in range(num_clustb):
    ax12.errorbar(perc_igmet[ecluster_label==i],
                  tau_c_optim[ecluster_label==i],yerr=t_c_err[:,ecluster_label==i],
                  ecolor=color_list[i],zorder=0,linestyle='')
    ax12.scatter(perc_igmet[ecluster_label==i],tau_c_optim[ecluster_label==i],
                  c=color_list[i],zorder=1,s=40)
ax12.set_ylabel(r'Optimized $\tau_c$')
ax12.set_yscale('log')
ax12.set_ylim((20,90))
